...
def analizar_sentimiento_intensidad(texto: str) -> Dict[str, Any]:
    ...
    Lógica de clasificación (Fase 7):
      1. Calcular similitudes coseno contra 3 anclas (positivo, negativo, neutro).
      2. Convertir a probabilidades vía softmax con temperatura 10.
      3. Calcular intensidad léxica y detectar es_evento_negativo (severidad/impacto).
      4. Reglas de decisión (en orden):
         a. Si es_evento_negativo → forzar 'negativo' (señal léxica fuerte).
         b. Si confianza < SENTIMENT_CONFIDENCE_THRESHOLD → forzar 'neutro'
            (el motor no tiene certeza; evitar argmax arbitrario en empates).
         c. En caso contrario → usar argmax de softmax.
      5. Si sentimiento final es 'neutro' → limitar intensidad a max 2.

    Esto resuelve el caso degenerado donde los 3 scores son iguales (ej. 0,0,0
    cuando el modelo no encuentra relación con ningún ancla) y argmax cae en
    'positivo' (primer índice) por convención, produciendo falsos positivos.
    """
    ...
    # 4. Ajustes por Reglas de Negocio (Fase 7: calibración por umbral de confianza)
    if es_evento_negativo and sentimiento_ganador != "negativo":
        # Señal léxica fuerte de negativo: forzar negativo
        sentimiento_ganador = "negativo"
        # La confianza podría recalibrarse, pero la dejamos igual para trazabilidad
    elif confianza < SENTIMENT_CONFIDENCE_THRESHOLD:
        # Confianza baja Y no hay señal léxica fuerte: forzar neutro
        # Esto evita que el argmax caiga arbitrariamente en 'positivo' cuando
        # los 3 scores son iguales o muy cercanos (empate degenerado).
        sentimiento_ganador = "neutro"

    # 5. Limitar intensidad si el resultado final es neutro
    if sentimiento_ganador == "neutro":
